...
class V4Controller extends AppController {
	public $uses = array('Plan', 'Location', 'ElectricityRate', 'GasRate', 'Tariff', 'ElectricityPostcodeDistributor', 'GasPostcodeDistributor', 'ElectricityNmiDistributor', 'Consumption', 'SolarRebateScheme', 'Customer', 'Sale', 'Submission');
			...
			case 3:
				...
				elseif ($filters['plan_type'] == 'Gas') {
					if ($distributor_gas) {
						...
						if ($distributor_elec['GasPostcodeDistributor']['sumo_power_distributor']) {
							$distributor_retailer_arr[] = 'Sumo Power';
						}
					}
				} 
		...
		if ($this->request->is('put') || $this->request->is('post')) {
			...
			switch ($step) {
				case 1:
					...
					$submission['BusOrResidential'] = ($data['customer_type'] == 'SME') ? 'Business' : 'Residential';
					...
					if ($this->request->data['sid']) {
                        ...
                        if (isset($this->request->data['wtcm_business']) && $this->request->data['wtcm_business'] == 'Yes') {
                            $campaign_id = 76;
                            if (!$first_campaign) {
                                $first_campaign = $submission['submitted']['first_campaign_name'] = 'WTCM Business - Campaign';
                            }
                        } else {
                            if (!$first_campaign) {
                        ...
                        if (isset($this->request->data['wtcm_business']) && $this->request->data['wtcm_business'] == 'Yes') {
                            $campaign_id = 76;
                            $first_campaign = $campaign_name = 'WTCM Business - Campaign';
                        }
