...
class V6Controller extends AppController {
	public $uses = array('Plan', 'Location', 'ElectricityRate', 'GasRate', 'Tariff', 'ElectricityPostcodeDistributor', 'GasPostcodeDistributor', 'ElectricityNmiDistributor', 'Consumption', 'SolarRebateScheme', 'Customer', 'Sale', 'Submission', 'LeadAgent', 'BroadbandLog', 'OffshoreLeadExceptions');
		...
		switch ($step) {
			...
			case 3:
				...
				if ($this->_view_top_picks && !empty($top_picks)) {
					...
					$conditions['Plan.version'] = array('All', '4', '5', '6');
				...
				if (!empty($plans_temp)) {
					...
					foreach ($plans_temp as $key => $plan) {
        				    ...
        				    if ($step1['campaign_source'] == 'Offshore') {
			...
			switch ($step) {
				case 1:
					...
					$data = array(
						...
						'renant_owner' => (isset($this->request->data['renant_owner'])) ? $this->request->data['renant_owner'] : '',
                    ...
                    $submission['submitted']['TenantOwner'] = $data['renant_owner'];
					...
					if ($this->request->data['sid']) {
                        ...
                        if ($campaign_id) {
                            switch ($campaign_id) {
                                case '76':
                                    if (!$first_campaign) {
                                        $first_campaign = $submission['submitted']['first_campaign_name'] = '13Energy Campaign';
                                    }
                                    break;
                                case '95':
                                    if (!$first_campaign) {
    	...
    	if ($this->request->is('put') || $this->request->is('post')) {
            ...
            if (isset($this->request->data['campaign_id2']) && $this->request->data['campaign_id2']) {
                ...
                switch ($campaign_id2) {
                    case '76':
                        $campaign_id = 76;
                        $first_campaign = $campaign_name = '13Energy Campaign';
                        break;
                    case '95':
                        $campaign_id = 95;
	...
	public function customer_details_update() {
    	if ($this->request->is('put') || $this->request->is('post')) {
            ...
            $submission_new['submitted']['last_comparison_link'] = 'https://tools.electricitywizard.com.au/v6/compare/1?customer='.$customer_key;
            $submission_new['submitted']['CompareStep1'] = 'https://tools.electricitywizard.com.au/v6/compare/1?customer='.$customer_key;
            $submission_new['submitted']['CompareStep2'] = 'https://tools.electricitywizard.com.au/v6/compare/2?customer='.$customer_key;
            $submission_new['submitted']['CompareStep3'] = 'https://tools.electricitywizard.com.au/v6/compare/3?customer='.$customer_key;
	...
	public function get_rates($details = false) {
		...
		if ($this->request->is('put') || $this->request->is('post')) {
			...
			if ($details) {
				$view_output = $view->render('/Elements/rate_details_v6');
			}
			else {
				$view_output = $view->render('/Elements/rates_v6');
			}
			return new CakeResponse(array(
				'body' => json_encode(array(
					'html' => $view_output
	...
	public function signup() {
		if ($this->request->is('put') || $this->request->is('post')) {
            ...
            if ($sid) {
        		$submission = array();
                $submission['submitted']['last_comparison_link'] = 'https://tools.electricitywizard.com.au/v6/compare/1?customer='.$customer_key;
                $submission['submitted']['CompareStep1'] = 'https://tools.electricitywizard.com.au/v6/compare/1?customer='.$customer_key;
                $submission['submitted']['CompareStep2'] = 'https://tools.electricitywizard.com.au/v6/compare/2?customer='.$customer_key;
                $submission['submitted']['CompareStep3'] = 'https://tools.electricitywizard.com.au/v6/compare/3?customer='.$customer_key;
                $this->update_lead($campaign_id, $sid, $submission);
    		}
	...
	private function create_lead($campaign_id = 1, $submission = array()) {
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://secure.velocify.com/Import.aspx?Provider=RSMSolutions&Client=RSMSolutions&CampaignId={$campaign_id}&XmlResponse=True");
	...
	private function add_lead_action($lead_id, $action_id) {
		...
		$action_note = urlencode('Tools v6 - Update');
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://service.leads360.com/ClientService.asmx/AddLeadAction?username={$username}&password={$password}&leadId={$lead_id}&actionTypeId={$action_id}&actionNote={$action_note}");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_HEADER, 0);
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
		$response = curl_exec($ch);
	...
	private function get_programs() {
		...
		$password = LEADS360_PASSWORD;
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://service.leads360.com/ClientService.asmx/GetPrograms?username={$username}&password={$password}");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_HEADER, 0);
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
		$response = curl_exec($ch);
	...
	private function assign_to_agent($lead_id, $agent_id) {
        ...
        $this->LeadAgent->save(array('LeadAgent' => array(
		    ...
		    'response' => $response,
		    'submitted' => date('Y-m-d H:i:s'),
		    'source' => 'Tools v6 - assign to agent',
        )));
	...
	public function form1() {
    	...
    	if ($this->request->is('put') || $this->request->is('post')) {
        	...
        	$submission['submitted']['TenantOwner'] = $this->request->data['renant_owner'];
